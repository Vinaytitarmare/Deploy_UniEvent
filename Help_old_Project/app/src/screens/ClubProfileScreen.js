...
export default function ClubProfileScreen({ route, navigation }) {
    ...
    useEffect(() => {
        ...
        const fetchClub = async () => {
            // ... (Logic remains same, redacted for brevity, will paste full logic below) ...
            try {
                let id = clubId;
                if (!id) {
    ...
    return (
        <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]} showsVerticalScrollIndicator={false}>
                        ...
                        <View style={styles.statItem}>
                            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 4 }}>
                                <Text style={[styles.statNum, { color: theme.colors.text }]}>
                                    {club?.reputation ? calculateAverageRating(club.reputation) : '—'}
                                </Text>
                                <Ionicons name="star" size={16} color="#FFD700" />
                            </View>
                            <Text style={[styles.statLabel, { color: theme.colors.textSecondary }]}>
                                {club?.reputation?.totalRatings ? `${club.reputation.totalRatings} ratings` : 'No ratings'}
                            </Text>
                        </View>
                    </View>
            ...
            <View style={styles.socialRow}>
                <TouchableOpacity onPress={() => openLink('https://instagram.com')} style={[styles.socialIcon, { backgroundColor: theme.colors.surface }]}>
                    <Ionicons name="logo-instagram" size={24} color="#E1306C" />
                </TouchableOpacity>
                <TouchableOpacity onPress={() => openLink('https://linkedin.com')} style={[styles.socialIcon, { backgroundColor: theme.colors.surface }]}>
                    <Ionicons name="logo-linkedin" size={24} color="#0077B5" />
                </TouchableOpacity>
                <TouchableOpacity onPress={() => openLink('https://twitter.com')} style={[styles.socialIcon, { backgroundColor: theme.colors.surface }]}>
                    <Ionicons name="logo-twitter" size={24} color="#1DA1F2" />
                </TouchableOpacity>
                <TouchableOpacity onPress={() => openLink('https://gdg.community.dev')} style={[styles.socialIcon, { backgroundColor: theme.colors.surface }]}>
                ...
                {activeTab === 'events' ? (
                    ...
                    <View style={styles.aboutContainer}>
                        ...
                        <View style={styles.contactRow}>
                            ...
                            <Text style={{ color: theme.colors.textSecondary }}>contact@{club?.displayName?.replace(/\s+/g, '').toLowerCase() || 'club'}.com</Text>
                        </View>
