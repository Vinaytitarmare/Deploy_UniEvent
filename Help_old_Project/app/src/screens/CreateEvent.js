...
export default function CreateEvent({ navigation }) {
    ...
    useEffect(() => {
        if (response?.type === 'success') {
            const access_token = response.authentication?.accessToken || response.params?.access_token;
            if (access_token) {
    ...
    const handleWebDateTimeChange = (value, isStart) => {
        if (!value) return;
    ...
    // Format date for web input (datetime-local format: YYYY-MM-DDTHH:mm)
    ...
    const validateForm = () => {
        ...
        if (!category) {
            Alert.alert('Missing Field', 'Please select an event category');
            return false;
        }

        if (eventMode === 'offline' && !location.trim()) {
            Alert.alert('Missing Field', 'Please enter a venue location for offline event');
            return false;
        }

        if (eventMode === 'online' && !meetLink.trim()) {
            Alert.alert('Missing Field', 'Please provide a Google Meet link for online event');
            return false;
        }

        if (isPaid) {
            if (!price || parseFloat(price) <= 0) {
                Alert.alert('Missing Field', 'Please enter a valid price for paid event');
                return false;
            }

            if (!upiId.trim()) {
                Alert.alert('Missing Field', 'Please enter UPI ID for payment collection');
                return false;
            }
        }
        ...
        <ScreenWrapper>
            <ScrollView contentContainerStyle={styles.container}>
                ...
                {Platform.OS === 'web' ? (
                    <TextInput
                        style={styles.input}
                        value={formatDateForWeb(startDate)}
                        onChangeText={(value) => handleWebDateTimeChange(value, true)}
                ...
                {Platform.OS === 'web' ? (
                    <TextInput
                        style={styles.input}
                        value={formatDateForWeb(endDate)}
                        onChangeText={(value) => handleWebDateTimeChange(value, false)}
                ...
                {isPaid && (
                    <View>
                        <TextInput
                            ...
                            placeholderTextColor={theme.colors.textSecondary}
                        />
                        <Text style={styles.label}>UPI ID (for payments) <Text style={{ color: 'red' }}>*</Text></Text>
                        <TextInput
                            style={styles.input}
                            value={upiId}
