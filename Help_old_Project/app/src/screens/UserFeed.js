...
export default function UserFeed({ navigation, headerContent }) {
    ...
    useEffect(() => {
        ...
        const unsubscribe = onSnapshot(q, (snapshot) => {
            ...
            console.log('📅 Fetched events:', list.length);
            console.log('📊 Display list will be:', list.map(e => e.title));
            setEvents(list);
            setLoading(false);
    ...
    const getFilteredEvents = () => {
        ...
        console.log('🎯 Returning filtered events:', filtered.length);
        return filtered;
    };
    ...
    console.log('🔍 Filtered displayList:', displayList.length);
    console.log('🔍 Active filter:', activeFilter);
    console.log('🔍 Search query:', searchQuery);
    console.log('🔍 User data:', userData?.name, userData?.department, userData?.year);

    const StickyHeader = () => (
        <View style={{ backgroundColor: theme.colors.background, paddingBottom: 10 }}>
            <View style={styles.headerContainer}>
                {/* Search Bar - Moved inside headerContainer for spacing or just removed titles */}
            </View>
    ...
    const renderHeader = () => (
        <Animated.View style={{ transform: [{ translateY: headerTranslateY }] }}>
            ...
            {/* Featured Carousel */}
            <FeaturedCarousel data={events.slice(0, 5)} />
        ...
        <View style={[styles.container, { backgroundColor: theme.colors.background }]}>
            {loading ? (
                <View style={{ paddingTop: 20 }}>
                    {/* Reuse Header for Skeleton State */}
                    <View style={styles.headerContainer}>
                        <View style={{ width: 100, height: 14, backgroundColor: theme.colors.border, marginBottom: 8, borderRadius: 4 }} />
                        <View style={{ width: 200, height: 32, backgroundColor: theme.colors.border, borderRadius: 4 }} />
                    </View>
                    <EventListSkeleton />
                </View>
            ) : (
