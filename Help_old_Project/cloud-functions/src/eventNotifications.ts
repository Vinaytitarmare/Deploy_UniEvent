 ...
 * Runs every minute.
 */
export const checkUpcomingEvents = functions.pubsub.schedule('every 1 minutes').onRun(async (context: functions.EventContext) => {
    ...
    for (const eventDoc of eventsSnapshot.docs) {
        ...
        const participantIds = participantsSnapshot.docs.map((doc: admin.firestore.QueryDocumentSnapshot) => doc.id);
        ...
        if (participantIds.length > 0) {
            ...
            const userDocs = await Promise.all(participantIds.map((uid: string) => db.collection('users').doc(uid).get()));
