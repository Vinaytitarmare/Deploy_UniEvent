...
const backupPath = path.join('node_modules', '@expo', 'cli', 'build', 'src', 'start', 'server', 'metro', 'externals_backup.js');
const targetPath = path.join('node_modules', '@expo', 'cli', 'build', 'src', 'start', 'server', 'metro', 'externals.js');
...
try {
  ...
  const safeCall = "if(!externalPath.includes('node:sea')) await fs.promises.mkdir(path.dirname(externalPath), { recursive: true });";
  ...
  const safeCall2 = "if(!externalPath.includes('node:sea')) await fs.promises.mkdir(externalPath, { recursive: true });";
